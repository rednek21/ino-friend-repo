...
class UserSearchForm(forms.Form):
    ...
    preferred_language = forms.ChoiceField(choices=UserQuestionnaireForm.get_languages(), widget=forms.Select(attrs={
        'class': 'form-select mini-text',
        'placeholder': 'Выберите изучаемый язык'
    }))
    gender = forms.ChoiceField(choices=choice_fields.GENDER_CHOICES, required=False)
    residence_city= forms.ChoiceField(choices=UserQuestionnaireForm.get_cities(),
                                       widget=forms.Select(attrs={
                                           'class': 'form-select mini-text',
                                           'placeholder': 'Выберите город'
                                       }), required=False)
    university = forms.ChoiceField(choices=UserQuestionnaireForm.get_universities(), widget=forms.Select(attrs={
        'class': 'form-select mini-text', 'aria-label': "Default select example",
        'placeholder': 'Выберите ВУЗ'
    }))
    specialty = forms.CharField(widget=forms.TextInput(attrs={
    ...
    class Meta:
        fields = ('hobby', 'preferred_language', 'gender', 'university',)
