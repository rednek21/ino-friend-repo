...
class ProfilesView(LoginRequiredMixin, CommonContextMixin, ListView):
    ...
    def user_check(request):
        ...
        if user.native_language is None:
            conditions_not_met.append("Родной язык")
    ...
    def get_queryset(self):
        queryset = User.objects.exclude(
            ...
            Q(gender=None) |
            Q(university=None) |
            Q(native_language=None) |
            Q(url__isnull=True) |
            Q(is_verified_email=False) |
            Q(preferred_language=None) |
        ...
        residence_city = self.request.GET.get('residence_city')
        if residence_city:
            queryset = queryset.filter(residence_city=residence_city)
...
class UserProfileEditView(LoginRequiredMixin, CommonContextMixin, UpdateView):
    ...
    def form_valid(self, form):
        ...
        if url.find('@') == -1 or url.find('@') != 0 or len(url) < 6:
            return self.form_invalid(form)
    ...
    def form_invalid(self, form):
        ...
        if image:
            if image.size <= 5 * 1024 * 1024:
                user.image = image
                user.save()
                return HttpResponseRedirect(reverse_lazy('users:profile_edit', args=(self.request.user.id,)))
            else:
                return super().form_invalid(form)
